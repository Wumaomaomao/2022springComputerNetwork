...
def Delete_low_traffic(table):
    traffic = 10000000
    delete_key = 0
    for key in table:
        if table[key][1] <= traffic:
            traffic = table[key][1]
            delete_key = key
...
def Update_forwardtable(forward_table, src_mac, dst_mac,fromIface):
    ...
    if src_mac in forward_table:
        ...
        if (len(forward_table) >= max_capcity):
            Delete_low_traffic(forward_table)
        forward_table[src_mac] = [fromIface, 0]

    if dst_mac in forward_table:
        forward_table[dst_mac][1] = forward_table[dst_mac][1] + 1
    for key in forward_table:
        log_info(f"(mac:{key},port:{forward_table[key][0]},traffic:{forward_table[key][1]})")   
